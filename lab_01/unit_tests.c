...
{
    int ec = 0;
    big_int_t a = bi_sread("12345678901234567890", &ec);
    big_int_t b = bi_sread("99999999999999999999", &ec);
    big_int_t c = bi_sub(&a, &b, &ec);
...
{
    int ec = 0;
    big_int_t a = bi_sread("23498757435712321983", &ec);
    big_int_t b = bi_sread("98752412358439545894", &ec);
    big_int_t c = bi_sub(&a, &b, &ec);
    big_int_t target = bi_sread("-75253654922727223911", &ec);
    assert(ec == 0);
    if (bi_cmp(&c, &target) != eq)
    {
        bi_print(&a);
...
void big_float_read_1()
{
    ...
    big_float_t target = {.m = {.p1=0, .p2=12345, .sign=0}, .exp=5};
    assert(ec == 0);
...
void big_float_read_2()
{
    ...
    big_float_t target = {.m = {.p1=0, .p2=12345, .sign=0}, .exp=2};
    assert(ec == 0);
    assert(bf_exact_eq(&a, &target));
}
...
void big_float_read_3()
{
    ...
    big_float_t a = bf_sread(a_str, &ec);
    big_float_t target = {.m = {.p1=0, .p2=12345, .sign=0}, .exp=-1};
    assert(ec == 0);
    assert(bf_exact_eq(&a, &target));
}
