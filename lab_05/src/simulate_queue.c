...
void simulate(simulation_times_t times, int queue_kind)
{
	...
	while (log.tasks_out_1 != target_tasks)
	{
		...
		if (time_until_finished == 0)
		{
			...
			time_until_finished = await(work_timespan);

			work_time += time_until_finished;
			log.tasks_out += 1;
		...
		float smallest_time = fminf(time_until_finished, time_until_fetch);
		time_until_finished -= smallest_time;
